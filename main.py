...
parser.add_argument('--Max_train_steps', type=int, default = 50000, help='Max training steps') #aslinya 5e6
...
parser.add_argument('--random_steps', type=int, default=20000, help='random steps before trianing')
    ...
    if opt.render:
        ...
        while total_steps < opt.Max_train_steps: # ini loop episode. Jadi total episode adalah Max_train_steps/200
            if lr_steps==sepertiga_eps :
                opt.a_lr=0.5 * opt.a_lr
                opt.c_lr=0.5 * opt.c_lr
                lr_steps=0
